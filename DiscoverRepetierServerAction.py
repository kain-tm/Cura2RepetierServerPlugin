...
class DiscoverRepetierServerAction(MachineAction):
    def __init__(self, parent = None):
        super().__init__("DiscoverRepetierServerAction", catalog.i18nc("@action", "Connect Repetier-Server"))

        self._qml_url = "DiscoverRepetierServerAction.qml"
        self._window = None
        self._context = None
    ...
    def startDiscovery(self):
        if not self._network_plugin:
            self._network_plugin = Application.getInstance().getOutputDeviceManager().getOutputDevicePlugin("RepetierServerPlugin")
    ...
    def _createAdditionalComponentsView(self):
        Logger.log("d", "Creating additional ui components for Repetier-Server - connected printers.")

        path = QUrl.fromLocalFile(os.path.join(PluginRegistry.getInstance().getPluginPath("RepetierServerPlugin"), "RepetierServerComponents.qml"))
        ...
        if not self._additional_components_view:
            Logger.log("w", "Could not create additional components for Repetier-Server - connected printers.")
            return

        Application.getInstance().addAdditionalComponent("monitorButtons", self._additional_components_view.findChild(QObject, "openRepetierServerButton"))
